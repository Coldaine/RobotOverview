# BEAST-01 Jetson flash and migration runbook — OP-JETSON-FLASH

**Moved out of [`beast-ops.md`](beast-ops.md) on 2026-08-07.** This is the record of a
*completed* one-time migration — Raspberry Pi 5 to Jetson Orin NX, R36.5 flash, NVMe
restore — together with the procedure for reflashing if that is ever needed again.

It is **reference and history, not current operating state.** `beast-ops.md` is the only
doc that describes how BEAST-01 runs now. Nothing here should be cited as a live fact.

This file also holds the last description of the superseded Raspberry Pi platform: the
vendor on-ramp of a web app on `:5000`, JupyterLab on `:8888`, and an optional ROS 2 stack
on `:5100`. **None of those ports listen on beast-01** (checked 2026-08-07 — only `22`,
`111`, `443`, and rosbridge on `127.0.0.1:9090`). The Pi was removed 2026-07-22; the Orin
runs ROS 2 Humble from systemd instead.

A `~8.8 V brownout on 2026-07-31` was removed from this text on 2026-08-07. It had no
measurement source, and `src/components/datacore/beast-console/reference-data.ts` had
already recorded on 2026-07-24 that the earlier brownout framing was a misreading.
...
watchdog). Charge the pack before the next motion session.
...
## Pi-era control surface — retired 2026-07-22 (moved from `beast-ops.md` 2026-08-07)

The sections below describe the Raspberry Pi control stack (Flask web app on `:5000`,
JupyterLab on `:8888`, Socket.IO `/json` + `/ctrl`). The Pi is out of the robot; none of
these endpoints exist. Kept because the ESP32 JSON payload shapes and the 2026-07-31
watchdog test are referenced from history.
