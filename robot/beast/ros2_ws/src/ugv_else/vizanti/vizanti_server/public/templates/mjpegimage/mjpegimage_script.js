...
const qualitySlider = document.getElementById('{uniqueID}_quality');
const qualityValue = document.getElementById('{uniqueID}_quality_value');
...
if(settings.hasOwnProperty("{uniqueID}")){
	...
	qualitySlider.value = loaded_data.quality;
	qualityValue.innerText = loaded_data.quality;
...
function connect(){
	...
	if(image_topic !== undefined){
		...
		image_topic = undefined;
		listener = undefined;
	}

	status.setWarn("Waiting for stream...");

	const stream_url = `http://${window.location.hostname}:8080/stream?topic=${topic}&type=mjpeg&quality=${qualitySlider.value}`;
	...
	canvas.onerror = () => {
		...
		displayImageOffset(img_offset_x, img_offset_y);
		status.setError("Stream error or unavailable.");
	};

	canvas.src = stream_url;
...
async function loadTopics(){
	let result = await rosbridge.get_topics("sensor_msgs/msg/Image");
	let topiclist = "";
...
console.log("Mjpeg Image Widget Loaded {uniqueID}")
