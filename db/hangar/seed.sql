...
INSERT INTO capabilities(id,name,description,unlocked) VALUES ('teleop','Remote Teleoperation','FPV remote piloting with onboard I/O on the Jetson Orin host and PID on the ESP32. Host is fitted and network-verified (2026-07-28); offline until the ESP32 UART jumper link is wired and the control software stack is confirmed on the Orin.',false);
...
-- insights + junctions
INSERT INTO insights(id,title,body,confidence,source,captured_at) VALUES ('usb-backfeeds-the-stack','The ESP32 USB cable powers the whole robot, not just its logic','On the ROS driver board the Type-C VBUS lands on net "5V" through Schottky D2 — the same net as both 40-pin headers, the LiDAR header and the AMS1117 input. So plugging the host into the ESP32 port energises the entire stack: HAT, codec, amp, fan and LiDAR motor, all through one USB connector, even with the chassis switch off. Confirmed on the bench by a D500 spinning with the pack dead. The board is designed to power the host, so it assumes the pack is on; running host-on/pack-off is not a supported state. Full trace in docs/beast-ops.md — OP-BEAST-BACKFEED.','high','docs/beast-ops.md — OP-BEAST-BACKFEED','2026-07-27');
...
INSERT INTO insights(id,title,body,confidence,source,captured_at) VALUES ('read-the-netlist-not-the-summary','Derived diagrams drift; go back to the netlist','A day of wrong diagnoses came from reasoning off a summarised power tree that said host VBUS reached only the 3V3 regulator. The schematic PDF''s embedded netlist said otherwise in one line, and Waveshare''s own product page — already sitting in keyArtifactstosort — answered the architecture question outright. When a hardware symptom contradicts the model, re-read the primary source before building a theory on the model. Vendor primaries are archived under keyArtifactstosort/reference/ with an INDEX noting what each answers.','high','keyArtifactstosort/reference/INDEX.md','2026-07-27');
...
INSERT INTO insights(id,title,body,confidence,source,captured_at) VALUES ('compute-sizing-method','Jetson tier fit needs linked workload views, not TOPS','Orin Nano / NX / AGX decisions are proven with concurrent pipelines: requirements → functional dataflow → cyber-physical allocation → interface/timing → engine matrix → full-graph measurements. Camera ingress, ISP, memory, encode, and sync often dominate over advertised TOPS. Full brief in Datacore.','high','/datacore/briefing/compute-workload','2026-07-23');
...
INSERT INTO insights(id,title,body,confidence,source,captured_at) VALUES ('rnd-splat-arch-b','Build A capture: images carry poses, LiDAR supplies geometry','Research (non-definitive): for offline Gaussian splatting, Architecture B wins — park the rover, stop-and-shoot, recover poses from the images (COLMAP/SfM), and use LiDAR for dense init / depth supervision. Sub-ms time sync, rigid cam-to-LiDAR mount, and hardware trigger are not load-bearing for this capture path. Full brief in Datacore.','medium','/datacore/briefing/beast-splat-architecture','2026-07-28');
...
INSERT INTO insights(id,title,body,confidence,source,captured_at) VALUES ('rnd-servo-cam-b0497','Servo-head research candidate: Arducam B0497 IMX678','Research (not purchased): Arducam B0497 (8.3MP STARVIS 2 IMX678, UVC USB3, ~$160) is the current pan-tilt head candidate under Architecture B. Three flags before any buy: stock focus is 3m–∞ (budget a closer M12 lens for indoors), integral IR-cut (visible only), 4K at 15fps only. Fixed chassis slot stays the owned OAK-D Lite.','medium','/datacore/briefing/beast-servo-camera','2026-07-28');
...
INSERT INTO insights(id,title,body,confidence,source,captured_at) VALUES ('rnd-vision-rejected','Zoom, thermal, FPV, and PTZ paths are research-killed','Research kill list for the current premise: DJI/Walksnail FPV chains, Insta360 X5, IP PTZ security cams, FLIR Lepton / InfiRay thermal, motorized optical zoom, and machine-vision trigger cameras. Do not re-propose without a new premise — prior console zoom/thermal shortlist is superseded. Full reasons in Datacore.','medium','/datacore/briefing/beast-rejected-paths','2026-07-28');
...
INSERT INTO insights(id,title,body,confidence,source,captured_at) VALUES ('rnd-lidar-open','Standalone LiDAR upgrade still open: Mid-360S vs Airy','Open research (not a ruling): Livox Mid-360S vs RoboSense Airy 96. Current lean is Mid for software lineage and Orin point-rate budget, but confidence is only medium. Servo-vs-rigid LiDAR mounting is a separate unresolved question for Build B nav SLAM.','medium','/datacore/briefing/beast-lidar-open','2026-07-28');
