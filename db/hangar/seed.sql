...
INSERT INTO assets(id,kind,name,manufacturer,model,callsign,status,lifecycle,provenance,flagship,summary,description,planning_notes,monitored_via,acquired,horizon,quantity,power_watts,power_volts,power_rail,mass_grams,price_us,price_import,specs,links,limitations,sources) VALUES ('orin-nano','module','Jetson Orin Nano Super',NULL,NULL,'BEAST-JETSON','integrating',NULL,'owner',false,'Owned NVIDIA Jetson Orin Nano 8GB developer kit — JetPack 6.2.2 / R36.5 on NVMe, ROS 2 Humble and the Jetson-adapted Waveshare workspace built. Physically fitted into BEAST-01 and network-verified by live SSH 2026-07-28 (hostname beast-01, LAN + Wi-Fi + Tailscale). Powered from the pack via the UPS board barrel-jack lead, not the driver board back-feed path. Remaining before full teleop: wire the ESP32 UART jumper link and finish the host mounting struts.',NULL,NULL,NULL,'owned',NULL,1,25,NULL,'battery',NULL,249,NULL,'[{"label":"AI perf","value":"up to 67 TOPS (Super); ~1.7× prior"},{"label":"Arch","value":"Ampere GPU + 6-core Arm"},{"label":"Power","value":"7–25 W"},{"label":"Memory","value":"8 GB"},{"label":"Cameras","value":"2× MIPI CSI (4-lane)"},{"label":"Carrier","value":"Accepts Orin Nano + Orin NX"},{"label":"Stack","value":"CUDA · TensorRT · Isaac ROS"},{"label":"Lifecycle","value":"through 2032"},{"label":"Cutover","value":"Fitted · network-verified · UART link pending"}]'::jsonb,'[{"label":"NVIDIA Jetson Orin","url":"https://www.nvidia.com/en-us/autonomous-machines/embedded-systems/jetson-orin/"}]'::jsonb,NULL,NULL);
...
INSERT INTO asset_shortcuts(asset_id,shortcut_id,position,label,type,url,command,note) VALUES ('beast','control-ui',0,'Control UI','url','http://192.168.20.184:5000',NULL,'Pre-cutover Pi endpoint — offline until Orin assumes beast identity');
INSERT INTO asset_shortcuts(asset_id,shortcut_id,position,label,type,url,command,note) VALUES ('beast','jupyterlab',1,'JupyterLab','url','http://192.168.20.184:8888',NULL,'Pre-cutover Pi endpoint — offline until Orin assumes beast identity');
INSERT INTO asset_shortcuts(asset_id,shortcut_id,position,label,type,url,command,note) VALUES ('beast','camera-stream',2,'Camera stream','url','http://192.168.20.184:5000/video_feed',NULL,'Pre-cutover Pi endpoint — offline until Orin assumes beast identity');
INSERT INTO asset_shortcuts(asset_id,shortcut_id,position,label,type,url,command,note) VALUES ('beast','ssh',3,'SSH','command',NULL,'ssh ws@192.168.20.184','Pre-cutover Pi identity — re-point after Orin DHCP/DNS cutover');
...
INSERT INTO insights(id,title,body,confidence,source,captured_at,bay) VALUES ('orin-host-gap','Pi out, Orin not in — Beast has no upper computer','2026-07-22: Raspberry Pi 5 removed from BEAST-01; host controller mount is empty while the software-ready Orin Nano Super is being fitted. No teleop/FPV until the Jetson is physically installed, UART/power/USB sensors are live, and beast network identity is moved off the old Pi. Active blocker is mechanical/host integration, not flash or ROS build.','high','Operator report 2026-07-22','2026-07-22','robotics');
