...
-- wishlist_meta
INSERT INTO wishlist_meta(asset_id,rationale,unlocks_capability_id,risk_note,for_asset_id,for_mission_id) VALUES ('light-bar','Wide flood, rugged, 2S–3S input — runs off the battery rail, not the Pi. Mount forward + angled down ~15–20° to light the floor without blasting dust at lens level.','night-ops','Import variants vary in driver quality; verify 3S rating.','beast','undercroft');
INSERT INTO wishlist_meta(asset_id,rationale,unlocks_capability_id,risk_note,for_asset_id,for_mission_id) VALUES ('crawler-led-kit','Cheapest path — multiple small LEDs you can place and offset individually off the lens axis.','night-ops',NULL,'beast','undercroft');
INSERT INTO wishlist_meta(asset_id,rationale,unlocks_capability_id,risk_note,for_asset_id,for_mission_id) VALUES ('depth-cam','Factory-calibrated RGB-D on one USB stream — erases camera intrinsics, extrinsics, and time-sync work. Computes depth onboard, so the host streams it without a GPU. The "reach high but solve power" first step.','rgbd-perception','Compromises max range vs standalone LiDAR.','beast','perimeter-mapping');
