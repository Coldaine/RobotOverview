...
INSERT INTO tags(namespace,name,label) VALUES ('tag','flash','flash') ON CONFLICT (namespace,name) DO NOTHING;
...
INSERT INTO insights(id,title,body,confidence,source,captured_at) VALUES ('jetson-flash-bct-boundary','Jetson flash is blocked before any storage write','OP-JETSON-FLASH proved the clean Ubuntu 22.04 VM, whole-controller VFIO path, APX enumeration, official R36.5 archive hashes, and every known USB buffer/power mitigation. Two read-only probes read the same ECID but timed out sending the first recovery BCT, before EEPROM retrieval, initrd boot, QSPI access, or NVMe access. The next retry must change a physical cable/port/controller and repeat a fresh recovery cycle; debug UART plus a bare-metal Ubuntu control is the escalation if that boundary repeats.','high','OP-JETSON-FLASH EVO whole-controller attempt ledger, 2026-07-11','2026-07-11');
INSERT INTO insight_assets(insight_id,asset_id) VALUES ('jetson-flash-bct-boundary','beast') ON CONFLICT DO NOTHING;
INSERT INTO insight_assets(insight_id,asset_id) VALUES ('jetson-flash-bct-boundary','orin-nano') ON CONFLICT DO NOTHING;
INSERT INTO insight_tags(insight_id,tag_id) SELECT 'jetson-flash-bct-boundary',id FROM tags WHERE namespace='tag' AND name='jetson' ON CONFLICT DO NOTHING;
INSERT INTO insight_tags(insight_id,tag_id) SELECT 'jetson-flash-bct-boundary',id FROM tags WHERE namespace='tag' AND name='flash' ON CONFLICT DO NOTHING;
INSERT INTO insight_tags(insight_id,tag_id) SELECT 'jetson-flash-bct-boundary',id FROM tags WHERE namespace='tag' AND name='operations' ON CONFLICT DO NOTHING;
INSERT INTO insight_tags(insight_id,tag_id) SELECT 'jetson-flash-bct-boundary',id FROM tags WHERE namespace='tag' AND name='usb' ON CONFLICT DO NOTHING;
...
-- activity_log
INSERT INTO activity_log(id,at,kind,text) VALUES ('a-jetson-flash-bct','2026-07-11T18:40:00Z','insight','OP-JETSON-FLASH built the EVO whole-controller flasher and isolated the current failure to the first read-only recovery BCT; no QSPI or NVMe write occurred.');
