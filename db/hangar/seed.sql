...
INSERT INTO insights(id,title,body,confidence,source,captured_at) VALUES ('jetson-flash-bct-boundary','Intel USB unblocked the Jetson R36.5 flash','OP-JETSON-FLASH proved the EVO AMD Strix Halo USB path caused the first-BCT timeout: the same signed transfer completed on the Intel workstation, which then flashed QSPI and the 2 TB Micron NVMe successfully. First boot verified beast-01 on Jetson Linux R36.5 with a 1.9 TiB NVMe root and key-based USB SSH. Later read-info probes can clobber bootloader RCM artifacts in a generated BSP tree, so regenerate and hash-check the RCM BCT against the package index before flash-only. Provisioning remains blocked only on repairing the intentionally discarded sudo credential, after which JetPack, Docker, ROS 2, ugv_ws, and Beast bench validation continue.','high','OP-JETSON-FLASH EVO whole-controller attempt ledger, 2026-07-11','2026-07-11');
...
-- activity_log
INSERT INTO activity_log(id,at,kind,text) VALUES ('a-jetson-flash-bct','2026-07-11T20:51:47Z','insight','OP-JETSON-FLASH flashed R36.5 to BEAST-JETSON QSPI and its 2 TB NVMe over Intel USB; first boot passed and sudo repair is the remaining provisioning blocker.');
