        ...
        if results:
            for r in results:
                ...
                try:
                    ...
                    R_base_cam = Rscipy.from_quat([q.x, q.y, q.z, q.w]).as_matrix()

                    # === 计算 base_link → tag ===
                    t_base_tag = R_base_cam @ t_cam_tag + t_base_cam

                    # 提取姿态和距离
                    x_b, y_b, z_b = t_base_tag.flatten()

                    error_x = center_x - 320
                    yaw_b = np.arctan2(error_x, self.K[0,0])

                    # yaw_b = math.atan2(R_base_tag[1, 0], R_base_tag[0, 0])
                    distance = math.sqrt(x_b**2 + y_b**2)
                    # 绘制辅助信息
                    corners = corners.astype(int)
                    cv2.polylines(frame, [corners], True, (0,255,0), 2)
                    cv2.putText(frame, f"dist={distance:.2f} yaw={yaw_b:.2f}",
                    ...
                    self.get_logger().info(
                        f"Tag wrt base_link: x={x_b:.2f}, y={y_b:.2f}, dist={distance:.2f}, yaw={yaw_b:.2f}"
                    )
                    ...
                    tf_msg.child_frame_id = "object_1"

                    tf_msg.transform.translation.x = float(t_base_tag[0])
                    tf_msg.transform.translation.y = float(t_base_tag[1])
                    tf_msg.transform.translation.z = float(t_base_tag[2])
