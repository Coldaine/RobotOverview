...
class ColorTrackPID(Node):
    def __init__(self):
        ...
        self.rotate_timer = self.create_timer(0.02, self.rotate_timer_callback)

        self.stop_count = 0
        self.last_has_line = True
    ...
    def rotate_timer_callback(self):
        """高频定时器控制旋转速度，保证旋转丝滑"""
        if not self.rotating or self.yaw is None or self.target_yaw is None:
            return
        ...
        # 比例控制角速度
        k_p = 1.5             # 可调节，控制旋转快慢
        max_w = 3.1416          # 最大角速度 rad/s (~90°/s)
        angular_speed = k_p * error
        angular_speed = max(-max_w, min(max_w, angular_speed))
    ...
    def image_callback(self, msg):
        ...
        # yellow
        self.lower_color = np.array([0, 100, 160])  
        ...
        if cx is not None:
            ...
            self.cmd_vel_pub.publish(twist)
            # self.call_pick_place(cmd=3)
            if self.last_has_line:
                self.get_logger().info("[Lost line] ❗首次检测到丢线事件")

                self.stop_count += 1

                if self.stop_count % 2 == 1:
                    self.get_logger().info(f"第 {self.stop_count} 次停止 → 夹取(cmd=3)")
                    self.call_pick_place(cmd=3)
                else:
                    self.get_logger().info(f"第 {self.stop_count} 次停止 → 放置(cmd=2)")
                    self.call_pick_place(cmd=2)

            # 当前帧标记为无线
            self.last_has_line = False
            # return
        # 显示
        gst_process.stdin.write(frame.tobytes())
        result_img_msg = self.bridge.cv2_to_imgmsg(frame, encoding="bgr8")
