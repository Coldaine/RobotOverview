...
function Unknown({ reason = "no publisher" }: { readonly reason?: string }) {
  return (
    <span
      className="inline-flex items-center gap-1 font-mono text-ink-dim/80"
...
export function SafetyStrip() {
  ...
  const robotConfirmed =
    status.hasReceived &&
    !status.stale &&
    status.muxSource === ESTOP_MUX_SOURCE;
  ...
  const assertingFor =
    intentLatched && estop.engagedAt !== null ? now - estop.engagedAt : 0;
  const asserting = intentLatched && !robotConfirmed;
  const assertStalled = asserting && assertingFor > ESTOP_CONFIRM_GRACE_MS;

  const estopMode: "offline" | "readonly" | "locked" | "asserting" | "clear" =
    !connected
      ? "offline"
      : !estop.writer
        ? "readonly"
  ...
  const estopCaption = {
    ...
    locked: estop.heartbeat
      ? "robot confirmed · holding 2 hz"
      : "robot confirmed · not holding",
  ...
  const voltPct =
    voltage === null
      ...
      : Math.max(
          0,
          Math.min(100, ((voltage - minVolts) / (maxVolts - minVolts)) * 100),
        );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col justify-center min-w-0 z-10">
        ...
        <span className="font-mono text-[10px] text-ink-dim truncate mt-1">
          {status.watchdogFired === true
            ? "WATCHDOG TRIGGERED"
            : "test: pending · ESP32 no FW HB"}
        </span>
      </div>
      ...
      <div className="flex flex-col justify-center min-w-0 z-10">
        ...
        >
          {status.muxSource === null ? (
            ...
            <span
              className={clsx(
                !status.stale && "text-cyan text-glow-cyan font-extrabold",
              )}
            >
              {status.muxSource}
            </span>
          ...
          <span className="text-sm font-medium ml-1">
            {status.cmdAge !== null && status.cmdAge >= 0
              ? `· ${status.cmdAge.toFixed(2)}s`
              : "· —"}
          </span>
        </span>
        <span className="font-mono text-[10px] text-ink-dim truncate mt-1">
      ...
      <div className="flex flex-col justify-center z-10 sm:col-span-2 md:col-span-1 min-w-0 col-span-1">
        ...
        <div className="relative h-2.5 rounded-full bg-hull border border-rim/60 overflow-visible mt-2">
          ...
          {/* Brownout @ 8.8V */}
          <div
            className="absolute top-[-3px] bottom-[-3px] w-[1px] bg-red-500"
            style={{ left: "0%" }}
            title="8.8V Brownout"
          >
            <span className="absolute top-[-11px] left-[-4px] font-mono text-[6px] text-red-500/80 font-bold scale-[0.8]">
              8.8
            </span>
          </div>
          {/* Floor @ 10.5V */}
          ...
          >
            <span className="absolute top-[-11px] left-[-4px] font-mono text-[6px] text-amber-500 font-bold scale-[0.8]">
              10.5
            </span>
          </div>
        </div>
