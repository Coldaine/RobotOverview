...
import { MISSION_STATUS_META, money, WISHLIST_STATUS_META } from '@/lib/format';
                ...
                {wishes.map((w) => (
                  <Link key={w!.id} href="/quartermaster" className="panel flex items-center justify-between p-3 transition-all hover:border-amber/40">
                    <div className="min-w-0">
                      ...
                      <span className={clsx('chip mt-1', WISHLIST_STATUS_META[w!.status].cls)}>
                        {WISHLIST_STATUS_META[w!.status].label}
                      </span>
                    </div>
